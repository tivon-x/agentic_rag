...
def build_retriever(settings: AppSettings) -> FusionRetriever | None:
    ...
    cfg, _ = resolve_indexer_config(settings)
    indexer = Indexer(cfg)
    ...
    nodes_path = Path(cfg.get("nodes_path", str(settings.nodes_path)))
    doc_trees_path = Path(
        cfg.get("doc_trees_path", str(settings.doc_trees_path))
    )
    if nodes_path.exists() and doc_trees_path.exists():
